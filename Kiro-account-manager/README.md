...
- **Cursor reversed model names** — Cursor sends model IDs version-first (`claude-4.6-sonnet-max-thinking`) instead of the canonical family-first form (`claude-sonnet-4.6`). The proxy now reorders these, strips the `-thinking`/`-reasoning` flag, and keeps the effort tier — so Opus no longer fails with `INVALID_MODEL_ID` and Sonnet/Opus no longer silently downgrade to Sonnet 4.5. Legacy 3.x aliases (`claude-3-opus`) are left untouched.
