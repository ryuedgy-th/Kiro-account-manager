...
- **Cursor model-name normalization** — Cursor decorates model IDs with effort + `thinking` markers in varying orders: version-first (`claude-4.6-sonnet-max-thinking`) and family-first with `-thinking` mid-string (`claude-opus-4-8-thinking-max`, its real Opus string). Model-name normalization is now token-based — it classifies family / version / effort / thinking segments regardless of order or position — so Opus no longer fails with `INVALID_MODEL_ID` (the mid-string `-thinking` used to leak to the backend), Sonnet/Opus no longer silently downgrade to Sonnet 4.5, and the user's chosen effort tier is preserved. Legacy 3.x aliases (`claude-3-opus`) are left untouched.
- **Credit exhausted → 402, not 429** — When a customer's credit runs out the proxy now returns `402 Payment Required` (a terminal billing error) instead of `429`. Clients like Claude Code / Cursor treated `429` as a transient rate-limit and retried forever without surfacing the real cause ("stuck retrying after top-up"). Real rate-limit / concurrency paths still use `429`.
- **Top-up actually unblocks customer keys** — A customer-bound key was also gated by its per-key `creditsLimit` (a lifetime usage cap that top-ups don't reset), so adding prepaid balance couldn't restore access. Customer keys are now gated by prepaid `creditBalance` only; standalone keys still honor `creditsLimit`.
- **Batch token refresh — transient-error retry** — "Refresh all" used to fail for random accounts with `fetch failed` while single per-account refresh worked. Firing many parallel TLS handshakes on a cold connection pool makes upstream reset some sockets (`UND_ERR_SOCKET`/`ECONNRESET`), and the refresh path had no timeout and no retry, so one transient connect-phase blip killed that account permanently. Refresh now uses a 15s per-attempt timeout plus up to 3 retries on transient network errors only (exponential backoff + jitter). Real auth failures (`401`/`403`/`invalid_grant`) are not retried. Covers every refresh path (batch, per-account, proxy server, proactive renewal) since they share the same refresh functions.
- **Document attachments without a name → no longer 500** — Sending a PDF/document with no `name`/`title`/`filename` (Anthropic's `title` and OpenAI's `filename` are both optional, and clients like Claude Code often send neither) used to fail the whole request with `500 "document requires name"`. The proxy now synthesizes a placeholder name from the document's media type (e.g. `document-1.pdf`) so attachments pass through, while explicit name/title/filename still win.
