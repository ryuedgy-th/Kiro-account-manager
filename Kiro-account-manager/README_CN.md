...
- **管理面登录后不跳转 Dashboard** — CSS 层叠 bug：`.hide` 输给了后面同优先级的规则，登录遮罩始终不隐藏。已用 `!important` 修复。

#### ✨ 改进
- **管理面充值历史** — 每个客户新增「ประวัติ」（历史）按钮，可查看人工 + slip2go 充值记录（时间、金额、来源、备注）。
- **页面模板可维护性** — 将门户/管理面 HTML 从代理源码中抽离为真正的 `.html` 文件（构建时内联）。编辑器现在按 HTML 校验，构建自动转义 `</script>`，白屏类问题无法再复发。
